  ...
  <div class="page">
    ...
    <el-row :gutter="20"><el-col :span="8"><el-card><h3 style="margin:0 0 16px;font-size:16px">开立客户账户</h3>
      <el-form ref="cFormRef" :model="cForm" :rules="cRules" label-width="80px">
        ...
        <el-form-item label="账户类型" prop="accountType"><el-select v-model="cForm.accountType" style="width:100%"><el-option label="个人" value="PERSONAL"/><el-option label="企业" value="CORPORATE"/></el-select></el-form-item>
    ...
    <el-col :span="8"><el-card><h3 style="margin:0 0 16px;font-size:16px">开立负债账户</h3>
      <el-form ref="lFormRef" :model="lForm" :rules="lRules" label-width="100px">
        ...
        <el-form-item label="产品类型" prop="accountType"><el-select v-model="lForm.accountType" @change="loadSubs" style="width:100%"><el-option label="活期存款" value="DEMAND"/><el-option label="定期存款" value="TIME"/></el-select></el-form-item>
...
var cFormRef=ref(),cForm=reactive({customerNo:'',accountType:'PERSONAL'}),cRules={customerNo:[{required:true,message:'请输入客户号',trigger:'blur'}],accountType:[{required:true,message:'请选择类型',trigger:'change'}]}
var lFormRef=ref(),lForm=reactive({customerAccountNo:'',accountType:'DEMAND'}),lRules={customerAccountNo:[{required:true,message:'请输入客户账号',trigger:'blur'}],accountType:[{required:true,message:'请选择产品类型',trigger:'change'}]}
var subs=ref([]),selSub=ref(null),closeForm=reactive({liabilityAccountNo:''}),closeFormRef=ref()
var loadSubs=async()=>{if(!lForm.customerAccountNo||!lForm.accountType){subs.value=[];return}try{var r=await getCustomerSubAccountByType(lForm.customerAccountNo,lForm.accountType);subs.value=(r.data||[]).filter(i=>i.status==='NORMAL')}catch(e){subs.value=[]}}
...
var openLiability=async()=>{await lFormRef.value.validate(async v=>{if(v){try{var d={...lForm};if(selSub.value)d.subAccountSeq=selSub.value.subAccountSeq;await openLiabilityAccount(d);ElMessage.success('开户成功');lForm.customerAccountNo='';lForm.accountType='DEMAND';selSub.value=null;subs.value=[]}catch(e){console.error(e)}}})}
