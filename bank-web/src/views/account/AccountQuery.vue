<template>
  <div class="page">
    ...
    <el-table :data="accountList" class="data-table" @row-click="showDetail">
      ...
      <el-table-column label="状态" width="130"><template #default="{ row: r }"><el-tag size="small" :type="r.status==='NORMAL'?'success':r.status==='FROZEN'?'danger':'info'" effect="light" style="border:0">{{{NORMAL:'正常',FROZEN:'冻结',CLOSED:'已销户',STOPPED:'停用'}[r.status]||r.status}}</el-tag></template></el-table-column>
    ...
    <el-drawer v-model="detailVisible" :title="'账户详情 - '+(detailData.liabilityAccountNo||'')" size="450px" destroy-on-close>
      <template v-if="detailData.liabilityAccountNo">
        ...
        <div class="detail-section"><div class="detail-section-title">账户状态</div>
          <el-descriptions :column="1" border size="small">
            <el-descriptions-item label="账户状态">{{{NORMAL:'正常',FROZEN:'冻结',CLOSED:'已销户',STOPPED:'停用'}[detailData.status]||detailData.status}}</el-descriptions-item>
