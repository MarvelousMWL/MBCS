  ...
  <div class="page">
    ...
    <el-card style="max-width:560px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="110px">
        ...
        <el-form-item label="产品类型" prop="accountType"><el-select v-model="form.accountType" @change="loadSubs" style="width:100%"><el-option label="活期存款 (DEMAND)" :value="0"/><el-option label="定期存款 (TIME)" :value="1"/></el-select></el-form-item>
...
var formRef=ref(),loading=ref(false),form=reactive({customerAccountNo:'',accountType:0}),subs=ref([]),selSub=ref(null)
...
var loadSubs=async()=>{if(!form.customerAccountNo||form.accountType===null){subs.value=[];return}try{var r=await getCustomerSubAccountByType(form.customerAccountNo,typeToSubType[form.accountType]);subs.value=(r.data||[]).filter(i=>i.status===0)}catch(e){subs.value=[]}}
var handleSubmit=async()=>{await formRef.value.validate(async v=>{if(v){loading.value=true;try{var d={...form};if(selSub.value)d.subAccountSeq=selSub.value.subAccountSeq;await openLiabilityAccount(d);ElMessage.success('负债账户开立成功');form.customerAccountNo='';form.accountType=0;selSub.value=null;subs.value=[]}catch(e){}finally{loading.value=false}}})}
</script>
