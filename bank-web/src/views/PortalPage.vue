  ...
  <div class="portal-page">
    ...
    <div class="portal-content">
      <header class="hero">
        ...
        <router-link to="/login" class="enter-btn">进入柜面系统 →</router-link>
      </header>
      ...
      <div class="system-grid">
        <router-link to="/login" class="system-card" @mousemove="onCardMove" @mouseleave="onCardLeave">
      ...
      <footer class="portal-footer">© 2026 MBCS Bank Core System</footer>
    </div>
  </div>
</template>
    ...
    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < 120) {
          ...
          ctx.strokeStyle = `rgba(109,140,255,${.05 * (1 - dist / 120)})`
...
.portal-page {
  ...
  background: #0a0e27; color: #e8ecf4;
  font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", "PingFang SC", "Microsoft YaHei", sans-serif;
  -webkit-font-smoothing: antialiased;
}
.bg-canvas { position: fixed; inset: 0; z-index: 0 }
.portal-content { position: relative; z-index: 1; max-width: 1200px; margin: 0 auto; padding: 80px 24px 60px }
...
.hero h1 { font-size: clamp(40px, 7vw, 64px); font-weight: 800; line-height: 1.1; letter-spacing: -2px; margin-bottom: 16px }
...
.hero p { font-size: 17px; color: #8892b0; max-width: 520px; margin: 0 auto 28px; line-height: 1.7 }
.enter-btn {
  display: inline-flex; align-items: center; gap: 8px;
  background: linear-gradient(135deg, #4f6ef7, #3651d5);
  color: #fff; border: none; border-radius: 12px; padding: 14px 36px;
  font-size: 16px; font-weight: 600; cursor: pointer; text-decoration: none;
  transition: all .3s; box-shadow: 0 4px 20px rgba(79,110,247,.35);
}
.enter-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(79,110,247,.5) }
...
.system-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-bottom: 60px }
.system-card {
...
.portal-footer { text-align: center; padding-top: 20px; border-top: 1px solid rgba(255,255,255,.06); color: #556; font-size: 13px }
