  ...
  <div class="page">
    ...
    <el-card style="max-width:560px;margin-bottom:20px">
      ...
      <el-form ref="subFormRef" :model="subForm" :rules="subRules" label-width="110px">
        ...
        <el-form-item label="产品编码" prop="productCode"><el-input v-model="subForm.productCode" placeholder="输入产品编码"/></el-form-item>
    ...
    <el-card>
      ...
      <el-table :data="subList" class="data-table">
        ...
        <el-table-column label="状态" width="100"><template #default="{ row: r }"><el-tag size="small" :type="r.status==='ACTIVE'?'success':'info'" effect="light" style="border:0">{{r.status==='ACTIVE'?'有效':'已兑付'}}</el-tag></template></el-table-column>
      </el-table>
    </el-card>
  </div>
</template>
...
var subForm=reactive({customerAccountNo:'',productCode:'',amount:0})
var subRules={customerAccountNo:[{required:true,message:'请输入客户账号',trigger:'blur'}],productCode:[{required:true,message:'请输入产品编码',trigger:'blur'}],amount:[{required:true,validator:(r,v,c)=>{if(v>0){c()}else{c(new Error('认购金额必须大于0'))}},trigger:'blur'}]}
var loadSubs=async()=>{try{var r=await getSubscribeList();subList.value=r.data||[]}catch(e){console.error(e)}}
var handleSubscribe=async()=>{await subFormRef.value.validate(async v=>{if(v){try{var r=await subscribeCertificate(subForm);ElMessage.success('认购成功! 存单账号:'+(r.data?.cdAccountNo||''));subForm.customerAccountNo='';subForm.productCode='';subForm.amount=0;loadSubs()}catch(e){console.error(e)}}})}
onMounted(()=>{loadSubs()})
</script>
