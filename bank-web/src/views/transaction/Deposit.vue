<template>
  <div class="page">
    <div class="page-header"><div><h2 class="page-title">存款交易</h2><p class="page-desc">向指定负债账户存入资金</p></div></div>
    <el-card style="max-width:560px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="存款金额" prop="amount"><el-input-number v-model="form.amount" :min="0.01" :precision="2" style="width:200px"/></el-form-item>
        <el-form-item><el-button type="primary" @click="handleSubmit">确认存款</el-button></el-form-item>
      </el-form>
    </el-card>
  </div>
</template>
...
import { deposit } from '../../api/transaction'; import { getCustomerSubAccountList } from '../../api/account'
...
var handleSubmit=async()=>{await formRef.value.validate(async v=>{if(v){try{var r=await deposit({liabilityAccountNo:form.liabilityAccountNo,amount:form.amount});ElMessage.success('存款成功! 流水号:'+(r.data?.transactionNo||''));form.customerAccountNo='';form.selectedSub=null;form.liabilityAccountNo='';form.amount=0;subList.value=[]}catch(e){console.error(e)}}})}
</script>
